...
fn main() {
    ...
    let input_path = args.get(0).expect("Expected input path!");
    let output_path = args.get(1).expect("Expected output path!");

    let mdl_file = gsparser::mdl::MdlFile::open(input_path);
    export(&mdl_file, output_path, None).expect("Error exporting gltf!");
    println!("Done!");
}